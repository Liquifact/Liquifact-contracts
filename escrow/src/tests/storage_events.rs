...
fn test_snapshot_event_not_emitted_when_below_target() {
    ...
    // Only init + funded events; no snap_st.
    let all = env.events().all();
    assert_eq!(
        all.events().len(),
        2,
        "expected exactly 2 events (init + funded), got {}",
        all.events().len()
    );
}
...
/// The snapshot event fires exactly once even when two investors are needed to cross the target.
/// The second investor's `fund` call must NOT re-emit it.
#[test]
fn test_snapshot_event_fires_once_on_two_investor_crossing() {
    ...
    // First deposit: below target — no snap_st yet.
    client.fund(&inv_a, &10_000i128);
    assert_eq!(
        env.events().all().events().len(),
        2, // init + funded_a
        "expected 2 events after first deposit"
    );

    // Second deposit: crosses target — snap_st must appear exactly once.
    ...
    // 4 events total: init + funded_a + snap_st + funded_b
    assert_eq!(list.len(), 4, "expected 4 events, got {}", list.len());
...
fn test_snapshot_event_not_emitted_when_target_still_above_funded() {
    ...
    // Expected: init + funded + fund_tgt = 3 events (no snap_st).
    let all = env.events().all();
    assert_eq!(
        all.events().len(),
        3,
        "expected 3 events (init+funded+fund_tgt), got {}",
        all.events().len()
    );
}
...
/// preserving causal ordering for indexers.
#[test]
