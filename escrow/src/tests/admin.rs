...
// ── set_registry_ref tests ───────────────────────────────────────────────────

/// Set the registry ref from None → Some, verify get_registry_ref returns the new value,
/// and assert the RegistryRefUpdated event carries the correct prior_registry (None) and
/// new_registry (Some).
#[test]
fn test_set_registry_ref_sets_from_none() {
    use soroban_sdk::testutils::Events as _;
    ...
    client.init(
        &admin,
        &soroban_sdk::String::from_str(&env, "SRR001"),
        &sme,
        &TARGET,
        &800i64,
        &0u64,
    ...
    let invoice_id = client.get_escrow().invoice_id.clone();
    client.set_registry_ref(&Some(registry.clone()));

    // Snapshot events immediately before any read that clears the event buffer.
    let set_events = env.events().all();
    assert_eq!(client.get_registry_ref(), Some(registry.clone()));

    // Verify event
    let last_event = set_events.events().last().unwrap().clone();
    assert_eq!(
        last_event,
        crate::RegistryRefUpdated {
            name: Symbol::new(&env, "reg_updated"),
            invoice_id,
            prior_registry: None,
            new_registry: Some(registry),
        }
        .to_xdr(&env, &contract_id)
    );
}

/// Overwrite an existing registry ref with a new one, verify the event carries
/// both the prior registry and the new value.
#[test]
fn test_set_registry_ref_overwrites_existing() {
    use soroban_sdk::testutils::Events as _;
    ...
    client.init(
        &admin,
        &soroban_sdk::String::from_str(&env, "SRR002"),
        &sme,
        &TARGET,
        &800i64,
        &0u64,
    ...
    let invoice_id = client.get_escrow().invoice_id.clone();
    client.set_registry_ref(&Some(reg2.clone()));

    // Snapshot events immediately before any read that clears the event buffer.
    let set_events = env.events().all();
    assert_eq!(client.get_registry_ref(), Some(reg2.clone()));

    // Verify event carries prior_registry = reg1 and new_registry = reg2
    let last_event = set_events.events().last().unwrap().clone();
    assert_eq!(
        last_event,
        crate::RegistryRefUpdated {
            name: Symbol::new(&env, "reg_updated"),
            invoice_id,
            prior_registry: Some(reg1),
            new_registry: Some(reg2),
        }
        .to_xdr(&env, &contract_id)
    );
}

/// Clear an existing registry ref to None, verify the event carries
/// prior_registry = Some and new_registry = None.
#[test]
fn test_set_registry_ref_clears_to_none() {
    use soroban_sdk::testutils::Events as _;
    ...
    client.init(
        &admin,
        &soroban_sdk::String::from_str(&env, "SRR003"),
        &sme,
        &TARGET,
        &800i64,
        &0u64,
...
/// Non-admin caller must be rejected by `load_escrow_require_admin`.
#[test]
#[should_panic]
fn test_set_registry_ref_requires_admin_auth() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);

    client.init(
        &admin,
        &soroban_sdk::String::from_str(&env, "SRR004"),
        &sme,
        &TARGET,
        &800i64,
        &0u64,
    ...
    // Clear all auth — admin not authorized
    env.mock_auths(&[]);
    client.set_registry_ref(&Some(Address::generate(&env)));
}

/// get_registry_ref must reflect the updated value after set_registry_ref.
#[test]
fn test_get_registry_ref_reflects_set_registry_ref() {
    let env = Env::default();
    env.mock_all_auths();
    let (client, admin, sme) = setup(&env);
    ...
    client.init(
        &admin,
        &soroban_sdk::String::from_str(&env, "SRR005"),
        &sme,
        &TARGET,
        &800i64,
        &0u64,
    ...
    client.set_registry_ref(&Some(reg.clone()));
    assert_eq!(client.get_registry_ref(), Some(reg.clone()));
