use soroban_sdk::{testutils::{Events, Address as _}, Env, Address, String, symbol_short, vec, IntoVal};
use crate::{EscrowError, tests::{init_and_fund_with_real_token, assert_contract_error, TARGET}, FinalRelease, PartialRelease};
...
fn test_release_zero() {
    ...
    assert_contract_error(client.try_release(&0), EscrowError::ReleaseAmountNotPositive);
}
...
fn test_release_exact_remaining() {
    ...
    assert_eq!(token_client.balance(&escrow_id), init_escrow_balance - TARGET);
...
fn test_release_above_remaining() {
    ...
    assert_contract_error(client.try_release(&(TARGET + 1)), EscrowError::ReleaseExceedsRemaining);
}
