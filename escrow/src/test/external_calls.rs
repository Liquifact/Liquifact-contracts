...
fn test_balance_delta_invariants_with_standard_token() {
    ...
    if holder_balance > 0 {
        token.token.transfer(&holder, &MuxedAddress::from(treasury.clone()), &holder_balance);
    }
    ...
    transfer_funding_token_with_balance_checks(
        &env, &token.id, &holder, &treasury, amount,
    );
    ...
    assert_eq!(spent, amount, "Sender balance delta must equal transfer amount");
    assert_eq!(received, amount, "Recipient balance delta must equal transfer amount");
    assert_eq!(holder_after, 0i128, "Sender should have zero balance after transfer");
    assert_eq!(treasury_after, amount, "Recipient should have exact transfer amount");
}
...
fn test_panics_with_zero_amount() {
    ...
    transfer_funding_token_with_balance_checks(
        &env, &token.id, &holder, &treasury, 0i128,
    );
}
...
fn test_panics_with_negative_amount() {
    ...
    transfer_funding_token_with_balance_checks(
        &env, &token.id, &holder, &treasury, -100i128,
    );
}
...
fn test_multiple_transfers_cumulative_balance_deltas() {
    ...
    // Verify final state
    assert_eq!(token.token.balance(&holder), initial_amount - total_transferred);
...
fn test_edge_case_maximum_amount_transfer() {
    ...
    transfer_funding_token_with_balance_checks(
        &env, &token.id, &holder, &treasury, large_amount,
    );
