...
/// Consolidated attestation snapshot returned by [`LiquifactEscrow::get_attestation_snapshot`].
/// Replaces the previous opaque tuple/log reads with a single typed struct.
#[contracttype]
#[derive(Clone, Debug, PartialEq)]
...
>>>>>>> pr-982
    ...
    /// Returns a consolidated snapshot of all attestation state in a single typed struct.
    /// Replaces the previous pattern of multiple opaque tuple/collection reads.
    pub fn get_attestation_snapshot(env: Env) -> AttestationSnapshot {
        let primary_hash = env.storage().instance().get(&DataKey::PrimaryAttestationHash);
        let append_log: Vec<BytesN<32>> = env.storage().instance().get(&DataKey::AttestationAppendLog).unwrap_or_else(|| Vec::new(&env));
        let log_len = append_log.len();
        let mut revoked_indices = Vec::new(&env);
        for i in 0..log_len {
            if env.storage().instance().get(&DataKey::AttestationRevoked(i)).unwrap_or(false) {
                revoked_indices.push_back(i);
            }
        }
        AttestationSnapshot { primary_hash, append_log, revoked_indices }
    }
